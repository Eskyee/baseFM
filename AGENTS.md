# baseFM - Complete Project Guide for Codex
...
## Git Workflow
```
1. AI (Codex/Copilot) works on a feature branch: copilot/<task-name> or Codex/<task-name>
...
AI agents (Codex/Copilot) create branches named `copilot/<task>` or `Codex/<task>`.
Each branch should:
...
- `atlas/rebuild-v2`
- `Codex/review-completion-status-QbcVX`
...
### Vercel Preview Flow
```
copilot/feature or Codex/feature
        ↓ (push)
...
## CI/CD Pipeline
`.github/workflows/ci.yml` runs on push to `main`, `Codex/**`, and `copilot/**` branches:
...
`.github/workflows/auto-pr.yml` runs on push to `Codex/**` and `copilot/**` branches:
...
21. **Delete AI branches after merge** - copilot/* and Codex/* branches are short-lived; delete them once the PR is merged to keep the repo tidy
